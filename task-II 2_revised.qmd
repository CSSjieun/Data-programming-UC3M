...
```{r}
#Answer
summary1 <- unesco_student |> 
  summarise(count = n(), .by = indicator) |> 
...
unesco_student |> filter(grepl("[A-Za-z]", country_code)) |> 
  slice_max(student_ratio, with_ties = FALSE, by = indicator)
  
unesco_student |> filter(grepl("[A-Za-z]", country_code)) |> 
  slice_min(student_ratio, with_ties = FALSE, by = indicator)

unesco_student |> filter(grepl("[A-Za-z]", country_code)) |> 
  slice_max(student_ratio, with_ties = FALSE, by = c(indicator, year))
  
unesco_student |> filter(grepl("[A-Za-z]", country_code)) |> 
  slice_min(student_ratio, with_ties = FALSE, by = c(indicator, year))
# 
# 
#   count(country_code) |>
...
unesco_student_sample <-
  unesco_student |>
  slice_sample(prop = 0.3, by = indicator)
unesco_student_sample
...
unesco_student |>
  count(indicator) |> 
  mutate(prop = n/nrow(unesco_student)*100)
...
sample <-
  unesco_student_filter |>
  mutate(probabilities = mean(student_ratio, na.rm = TRUE), .by = year) |>
  slice_sample(prop = 0.1, by = year, weight_by = probabilities)
sample
...
```{r}
sample |>
  slice_max(year, by = c(country_code, indicator))
```
...
tidy_data <-
  ...
  pivot_longer("BERDEAK-LOS VERDES":"COALICIÓN POR MELILLA", names_to = "party", values_to = "votes", values_drop_na = TRUE)
tidy_data
```
...
tidy_data3 <- 
  ...
  mutate(id_mun = glue('{codigo_ccaa}-{codigo_provincia}-{codigo_municipio}'),
        .after = date) |> 
  group_by(date) |>
  distinct(id_mun, party, .keep_all = TRUE) |>
  ungroup()
tidy_data3
```
...
tidy_data4 <-
  ...
  mutate(votes = sum(votes), .by = c(date, id_mun, acronym)) |> 
...
tidy_data5 <-
  tidy_data4 |>
  group_by(date, id_mun, acronym) |> 
  mutate(percent = 100*votes / (votos_blancos + votos_nulos + votos_candidaturas))
...
tidy_data5 |> 
  slice_max(date) |>
  slice_max(n = 2, percent, by = id_mun)
  
  # filter(date == "2019-04-01" | date == "2019-11-01") |> 
...
```{r}
tidy_data5 |>
  group_by(date, codigo_provincia, acronym) |> summarise(sum_vote = sum(votes))

tidy_data5 |> group_by(date, codigo_ccaa, acronym) |> summarise(sum_vote = sum(votes))

tidy_data5 |> group_by(date, acronym) |> summarise(sum_vote = sum(votes))
```
...
```{r}
tidy_data5 |> select(id_mun, censo, acronym, votes, date) |> 
  distinct(date, id_mun, .keep_all = TRUE) |> slice_max(censo, n=7) |> 
  slice_max(percent)
